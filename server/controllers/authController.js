...
async function register(req, res){
    ...
    if (password !== password_confirm) return res.status(400).json({ 'message': 'Passwords do not match' });
...
async function login(req, res){
    ...
    await user.save()
    //add , secure: true while using frontend
...
async function logout(req, res){
    ...
    if(!user){
        //add , sameSite: 'None', secure: true while using frontend
